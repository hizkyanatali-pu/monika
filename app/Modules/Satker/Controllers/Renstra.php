...
class Renstra extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->dokumenSatker          = $this->db->table('data_renstra');
    ...
    public function index()
    {
        ...
        $query_dataDokumen = $this->dokumenSatker->select('
            data_renstra.id,
            data_renstra.template_id,
            data_renstra.revision_master_dokumen_id,
            data_renstra.revision_master_number,
            data_renstra.revision_number,
            data_renstra.status,
            (CASE
            WHEN data_renstra.acc_by IS NULL THEN data_renstra.reject_by
            ELSE data_renstra.acc_by
            END) AS verif_by,
            
            data_renstra.is_revision_same_year,
            data_renstra.change_status_at,
            data_renstra.created_at,
            dokumen_pk_template_' . session('userData.tahun') . '.title as dokumenTitle
        ')
            ->join('dokumen_pk_template_' . session('userData.tahun'), 'data_renstra.template_id = dokumen_pk_template_' . session('userData.tahun') . '.id', 'left')
            // ->where('user_created', $this->userUID)
            ->where('data_renstra.status !=', 'revision')
            ->where("data_renstra.deleted_at is null")
            ->where("data_renstra.tahun", $this->user['tahun'])
            ->orderBy('data_renstra.id', 'DESC');
