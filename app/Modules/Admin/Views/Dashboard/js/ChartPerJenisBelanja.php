<script>
    var d1_1 = [
        <?php
        $index = 1325376000000;
        foreach ($jenisbelanja as $key => $value) :
    ...
    var jenisbelanja = $.plot($("#chatperjenisbelanja"), data1, {
        xaxis: {
            mode: "time",
            timeformat: "%b",
            tickSize: [1, "month"],
        ...
        legend: {
            show: true,
            noColumns: 5,
            container: "#bar-legend-jenis-belanja"
        },
        tooltip: true,
        tooltipOpts: {
            id: "chart-tooltip",
    ...
    $.each(jenisbelanja.getData()[0].data, function(i, el) {
        var o = jenisbelanja.pointOffset({
            x: el[0],
            y: el[1]
        });
        $('<div class="data-point-label">' + el[1] + '%' + '</div>').css({
            ...
            top: o.top - 20,
            display: 'none'
        }).appendTo(jenisbelanja.getPlaceholder()).fadeIn('slow');
    });

    $.each(jenisbelanja.getData()[1].data, function(i, el) {
        var o = jenisbelanja.pointOffset({
            x: el[0],
            y: el[1]
        });
