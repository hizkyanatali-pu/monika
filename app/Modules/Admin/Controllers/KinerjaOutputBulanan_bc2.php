...
class KinerjaOutputBulanan extends \App\Controllers\BaseController
{
    ...
    public function index($bulan = '')
    {
        ...
        pkt.kdprogram,pkt.kdgiat,pkt.kdoutput,pkt.kdsoutput,pkt.kdkmpnen,tp.nmprogram,tgiat.nmgiat,toutput.nmoutput,pkt.sat,
        pkt.pg,CAST(REPLACE(pkt.vol,',','.') AS DECIMAL) vol
        --,pkt.kdprogram || '.' || pkt.kdgiat || '.' || pkt.kdoutput || '.' || pkt.kdsoutput || '.'|| pkt.kdkmpnen AS kode
        ,pkt.kdprogram || '.' || pkt.kdgiat || '.' || pkt.kdoutput || '.' || pkt.kdsoutput AS kode
        ...
        WHERE pkt.kdprogram = 'FC' ORDER BY pkt.kdprogram,pkt.kdgiat,pkt.kdoutput,pkt.kdsoutput")->getResultArray();
