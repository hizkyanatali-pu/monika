...
class Importdata extends \App\Controllers\BaseController
{
    ...
    function pullimport($type = '')
    {
        ...
        // exit;
        if ($type == 'paket') {
            $data = file_get_contents(getenv('API_EMON') . "api_sda/paket?thang=" . $tahunAnggaran);
            $nmFile = date("ymdHis") . '_fromemon_paket_' . $tahunAnggaran;
            $regex = preg_replace('/\s+/', ' ', $data);
        } else if ($type == 'kontrak') {
            $data = file_get_contents(getenv('API_EMON') . "api_sda/kontrak?thang=" . $tahunAnggaran);
            ...
            $data = file_get_contents(getenv('API_EMON') . "api_sda/paket_register?thang=" . $tahunAnggaran);
            $nmFile = date("ymdHis") . '_fromemon_paket_register_' . $tahunAnggaran;
            $regex = preg_replace('/\s+/', ' ', $data);
        } else {

            $data = file_get_contents(getenv('API_EMON') . "api_sda/rekap_unor?thang=" . $tahunAnggaran);
