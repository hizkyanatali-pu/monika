...
class Pulldata extends \App\Controllers\BaseController
{
    ...
    public function rekap($pg)
    {
        ...
        if($pg=="paket"){
            ...
            if($hal['paket']['format']=='db'){$pgview ="Rekap-3";}
        }
        // d($pgview);
        // dd($hal);

        // foreach ($hal as $key => $value) {$hall[]=$key;}
        if(!in_array($pg, $hall)){ return redirect()->to('/pulldata/nopage/'.$pg);}

        $fileName="rekapMonika-".$hal[$pg]['title']."-". date('Ymdhis');
        $data=$hal[$pg];
        $data['qdata'] = ($pg=="paket" ? $this->PulldataModel->getPaket($hal[$pg]['where']) :
